...
class ApplicationMenu extends Component<Props> {
  ...
  openGettingStartedDocs = () => {
    shell.openExternal(`${SKPM_REPO_URL}/blob/master/docs/getting-started.md`);
  };

  openReportIssue = () => {
    shell.openExternal(`${SKPM_REPO_URL}/issues/new/choose`);
  };
...
const mapStateToProps = state => {
  ...
  const devServerTask = selectedProject
    ? getDevServerTaskForProjectId(state, selectedProject.id)
    : null;
