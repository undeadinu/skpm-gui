...
describe('task saga', () => {
  ...
  describe('launchDevServer saga', () => {
    ...
    const { args, env } = getDevServerCommand(task);
    ...
    it('should spawn a child process', () => {
      expect(saga.next(task).value).toEqual(call(getDevServerCommand, task));
  ...
  describe('taskAbort saga', () => {
    ...
    it('should display correct message for dev server task', () => {
      ...
      expect(saga.next().value).toEqual(
        put(
          receiveDataFromTaskExecution(task, chalk.bold.red('Dev mode stopped'))
        )
      );
